        ...
        <div class="container-fluid">
			<table id ="example" class="table table-hover table-stripped">
				...
				<tbody>
					...
					if ($agent=='*'){
						$qry = $conn->query("SELECT * from withdrawals where active ='N' and user_id IN (Select id from users where parentid = '{$_settings->userdata('id')}'    ) ");
						}else{
						$qry = $conn->query("SELECT * from withdrawals where active ='N' and user_id IN (Select id from users where parentid = '{$_settings->userdata('id')}'  and usercode ='{$agent}'  ) ");	
					}                       
						...
						<?php endwhile; ?>
						<h6><strong>Total Withdrawals: <?php echo number_format($bal,2) ?></strong></h6>
				</tbody>
			</table>
		</div>
		</div>
...
<script>
	$(document).ready(function(){
        ...
        $('#filter').click(function(){
			location.replace("./?page=maintenance/hist_withdrawals&agent="+($('#agent').val()) );
        })
                $('#example').DataTable( {
                stateSave: true
                } );
