        ...
        <div class="container-fluid">
			<table class="table table-stripped table-hover" id="example">
				...
				<tbody>
					<?php 
					$i = 1;
						$qry = $conn->query("SELECT * from `holidays` order by name asc ");
						while($row = $qry->fetch_assoc()):
                            $row['description'] = strip_tags(stripslashes(html_entity_decode($row['description'])));
					?>
						<tr title="<?php echo $row['description'] ?>">
							<td class="text-center"><?php echo $i++; ?></td>
							<td><?php echo (date('m-d-Y',strtotime($row['name']))); ?></td>
...
<script>
	$(document).ready(function(){
		$('.delete_data').click(function(){
			_conf("Are you sure to delete this date permanently?","delete_holiday",[$(this).attr('data-id')])
		})
		$('.edit_data').click(function(){
			uni_modal("<i class='fa fa-edit'></i> Edit Holiday Details",'maintenance/manage_holiday.php?id='+$(this).attr('data-id'))
		})
		$('#create_new').click(function(){
			uni_modal("<i class='fa fa-plus'></i> Create New Office Holiday",'maintenance/manage_holiday.php')
		})
		$('#example').DataTable( {
                stateSave: true
                } );
