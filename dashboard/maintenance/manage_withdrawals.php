...
if(isset($_GET['id']) && $_GET['id'] > 0){
    $qry = $conn->query("SELECT * from `withdrawals` where id = '{$_GET['id']}' ");
    if($qry->num_rows > 0){
...
<div class="container-fluid">
	<form action="" id="withdrawal-form">
        ...
        <?php if($_settings->userdata('type') == 1): ?>
		<a class="btn btn-flat btn-default" href="?page=maintenance/withdrawals&status=<?php echo $status ?>&agent=<?php echo $agent ?>">Cancel</a>
        <?php else: ?>
        <a class="btn btn-flat btn-default" href="?page=maintenance/withdrawals">Cancel</a>
        <?php endif; ?> 
	</div>
	...
	$(document).ready(function(){
		...
		e.preventDefault();
	       ...
	       $('.err-msg').remove();               
            start_loader();
		$.ajax({
		url:_base_url_+"classes/Master.php?f=save_withdrawals",
		data: new FormData($(this)[0]),
				...
				success:function(resp){
					...
					if(typeof resp =='object' && resp.status == 'success'){
                        <?php if($_settings->userdata('type') == 1): ?>
				            location.href = "./?page=maintenance/withdrawals&status="+ex+"&agent="+ag;   
                        <?php else: ?>  
                            location.href = "./?page=maintenance/withdrawals";     
                        <?php endif; ?>    
