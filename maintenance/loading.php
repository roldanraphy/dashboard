...
<div class="card card-outline card-primary">
	<div class="card-header">
		...
		<a href="?page=maintenance/manage_loading&status=<?php echo $status ?>&agent=<?php echo $agent ?>" class="btn btn-flat btn-primary"><span class="fas fa-plus"></span>  Create New</a>
		<?php else: ?>
		<a href="?page=maintenance/manage_loading" class="btn btn-flat btn-primary"><span class="fas fa-plus"></span>  Create New</a>			 
		<?php endif; ?> 
		</div>
	</div>
	<div class="card-body">
        ...
        <div class="container-fluid">
			<table id ="example" class="table table-hover table-stripped">
				...
				<tbody>
					...
					$i = 1;
					$bal = 0;		
					$mybal = $conn->query("SELECT amount from users where id = '{$_settings->userdata('id')}' ");
					while($row1 = $mybal->fetch_assoc()){
						$bal = number_format($row1['amount'],2);
					}
				...
				if ($_settings->userdata('type') == 1){ //use admin priv                            
                    if ($status=='*'){
						if ($agent=='*'){
                        	$qry = $conn->query("SELECT * from loading");
						}else{
							$qry = $conn->query("SELECT * from loading where user_id = '$agent'");
						}
                    }
                    elseif ($status=='Y'){
						if ($agent=='*'){
                        	$qry = $conn->query("SELECT * from loading where active ='Y'");
						}else{
							$qry = $conn->query("SELECT * from loading where active ='Y' and user_id = '$agent'");							
						}
						                
                    }
                    else{
						if ($agent=='*'){
                        	$qry = $conn->query("SELECT * from loading where active ='N'");   
						}else{
							$qry = $conn->query("SELECT * from loading where active ='N' and user_id = '$agent'");  
						}
                    }
                      
					
				}else{ //admin priv
					//$qry = $conn->query("SELECT * from loading where active ='Y' and user_id = '{$_settings->userdata('id')}'");
					$qry = $conn->query("SELECT * from loading where active ='Y' and user_id IN (Select id from users where parentid = '{$_settings->userdata('id')}') ");
						...
						while($row = $qry->fetch_assoc()):
                            ...
                            Select<span class="sr-only">Toggle Dropdown</span>
				                  ...
				                  <div class="dropdown-menu" role="menu">
								  	...
								  	<?php if($_settings->userdata('type') == 1): ?>  								
				                    <a class="dropdown-item" href="?page=maintenance/manage_loading&id=<?php echo $row['id'] ?>&status=<?php echo $status ?>&agent=<?php echo $agent ?>"><span class="fa fa-edit text-primary"></span> Edit</a>
						...
						<?php endwhile; ?>
						<h5><strong>Current Balance: <?php echo $bal ?></strong></h5>
				</tbody>
			</table>
		</div>
		</div>
...
<script>
	$(document).ready(function(){
        ...
        $('#filter').click(function(){
			location.replace("./?page=maintenance/loading&status="+($('#status').val())+"&agent="+($('#agent').val()) );
        })
		...
		$('.post_all').click(function(){
			_conf("Do you want to POST all active load requests?","post_all")
		})
	...
	function delete_transaction($id){
		start_loader();
		$.ajax({
			url:_base_url_+"classes/Master.php?f=delete_loading",
			method:"POST",
			data:{id: $id},
			dataType:"json",
	...
	function post_all(){
		start_loader();
		$.ajax({
			url:_base_url_+"classes/Master.php?f=postall_load",
			method:"POST",
			dataType:"json",
			error:err=>{
				console.log(err)
