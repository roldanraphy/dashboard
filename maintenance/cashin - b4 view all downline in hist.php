...
if($_settings->userdata('type') == 2){
	$rec = $conn->query("SELECT GROUP_CONCAT(lv SEPARATOR ',') as id #lv
	FROM (SELECT @pv :=(SELECT GROUP_CONCAT(id SEPARATOR ',') FROM users WHERE FIND_IN_SET(parentid, @pv)) AS lv 
	FROM users JOIN (SELECT @pv := '{$_settings->userdata('id')}') tmp) tmp2"); 
	if($rec->num_rows > 0){
		$rowx = $rec->fetch_assoc();
		if(!is_null($rowx['id'])){ //palaging me
			$arr = explode(',', $rowx['id']); 
			$count=0;
			foreach($arr as $myvalue) {
				if ($count == 0){
					$mysql="select id, date_created,1 as type, amount,user_id,description,agent_id from loading where user_id=". $myvalue;
					$mysql1="select id, date_created,2 as type, amount,user_id,description,agent_id from withdrawals where user_id=". $myvalue;
				}else{
					$mysql .= " or user_id= ".$myvalue;
					$mysql1 .= " or user_id= ".$myvalue;
				}
			$count++;	
			}
		}else{
			$mysql="select id, date_created,1 as type, amount,user_id,description,agent_id from loading where user_id=0";
			$mysql1="select id, date_created,2 as type, amount,user_id,description,agent_id from withdrawals where user_id=0";
		}
		$mysql .= " and active='N' ";
		$mysql1 .= " and active='N' ";
	}
	...
	$mysql="select id, date_created,1 as type, amount,user_id,description,agent_id from loading where active='N'";
	$mysql1="select id, date_created,2 as type, amount,user_id,description,agent_id from withdrawals where active='N' ";
}
$cursql=$mysql." UNION ". $mysql1;
					...
					if ($agent=='*'){
						if($_settings->userdata('type') == 1){
							...
							$qry = $conn->query("SELECT * from loading where active ='N' and agent_id ='{$_settings->userdata('id')}'");
						}
					}else{
						if($_settings->userdata('type') == 1){
							$qry = $conn->query("SELECT * from loading where active ='N' and user_id ='{$agent}'");
						}else{
							$qry = $conn->query("SELECT * from loading where active ='N' and agent_id ='{$_settings->userdata('id')}' and user_id ='{$agent}'");
						}
					}
