...
class SystemSettings extends DBConnection{
	...
	function update_settings_info(){
		...
		if(isset($_FILES['left_img']) && $_FILES['left_img']['tmp_name'] != ''){
			$fname = 'uploads/'.strtotime(date('y-m-d H:i')).'_'.$_FILES['left_img']['name'];
			$move = move_uploaded_file($_FILES['left_img']['tmp_name'],'../'. $fname);
			if(isset($_SESSION['system_info']['left_img'])){
				$qry = $this->conn->query("UPDATE system_info set meta_value = '{$fname}' where meta_field = 'left_img' ");
				if(is_file('../'.$_SESSION['system_info']['left_img'])) unlink('../'.$_SESSION['system_info']['left_img']);
			}else{
				$qry = $this->conn->query("INSERT into system_info set meta_value = '{$fname}',meta_field = 'left_img' ");
			}
		}
